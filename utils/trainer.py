                    ...
                    for name, param in model.named_parameters():
                        if param.requires_grad:
                            ...
                            if torch.isinf(param.grad).any():
                                print(f"Parameter {name} has Inf gradients")
                            # clip_value = torch.quantile(param.grad.view(-1).abs(), percentile / 100.0)
                            std_dev = torch.std(param.grad.view(-1))
                            clip_value = 3 * std_dev
                            nn.utils.clip_grad_value_(param, clip_value)
