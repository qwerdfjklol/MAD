...
from utils.transformer.brain_transformer import BrainTransformer
from utils.brain_module import ClipLoss, SimpleConv
from utils.loss import MMDLoss
from dataclasses import dataclass
from transformers import PreTrainedModel, WhisperProcessor
from typing import Optional, Tuple, Union
from tllib.alignment.jan import JointMultipleKernelMaximumMeanDiscrepancy, Theta
...
class BrainWhisperForConditionalGeneration2(PreTrainedModel): #nn.Module
    ...
    def __init__(self, config, loss_dict, pretrained_layers, run_name, depth):
        ...
        self.brain_module = BrainTransformer(in_channels={"meg": self.config.modal_ch}, run_name=self.config.run_name, depth=self.config.depth, useful_length=1200).to(self.model.device)
        self.processor = WhisperProcessor.from_pretrained('openai/whisper-base',
                                                    language='en',
                                                    task='transcribe',
                                                    local_files_only=False,
                                                    hop_length=160)
        self.thetas = [Theta(dim).to(self.model.device) for dim in (1500, 1500, 1500)]
        self.theta_mel = [Theta(400).to(self.model.device)]
        self.jmmd_loss = JointMultipleKernelMaximumMeanDiscrepancy(
            kernels=(GaussianKernel(), GaussianKernel(), GaussianKernel()),
            linear=False, thetas=self.thetas
        ).to(self.model.device)
        self.jmmd_mel_loss = JointMultipleKernelMaximumMeanDiscrepancy(
    ...
    def predict_mel(self,input_features,useful_length,subject_index):
        seq_len = 3000
        pred_mel, ctc_out = self.brain_module({"meg": input_features[..., :useful_length]}, {"subject_index": subject_index})
        ...
        max_values_batch_pred = torch.max(pred_mel.flatten(start_dim=1, end_dim=2), dim=1).values
        # print(max_values_batch)
        # Make sure max_values_batch has shape [batch_size, 1]
        max_values_batch_pred = max_values_batch_pred.unsqueeze(-1).unsqueeze(-1)

        # Use torch.clamp to limit the value of pred_mel to the range [minimum value, max_values_tensor]
        pred_mel = torch.clip(pred_mel, min=max_values_batch_pred - 2, max=max_values_batch_pred)
        return pred_mel, ctc_out
    ...
    def forward(
        ...
        for theta in self.thetas:
            theta = theta.to(self.model.device)
        self.jmmd_loss = self.jmmd_loss.to(self.model.device)
        self.theta_mel[0] = self.theta_mel[0].to(self.model.device)
        self.jmmd_mel_loss = self.jmmd_mel_loss.to(self.model.device)
        useful_length = 1200
        ...
        '''
        pred_mel, ctc_out =self.predict_mel(input_features=input_features, # 100*4
        ...
        if isinstance(self.config.total_loss, list):
            ...
            if labels is not None:
                ...
                if self.config.total_loss['mmd'] != 0:
                    with torch.no_grad():
                        mel_input_encoder_outputs = self.get_encoder_output(input_features=mel_spec, attention_mask=attention_mask,
                                                                ...
                                                                output_hidden_states=output_hidden_states)
                        mel_input_encoder_outputs_last_hidden_state=mel_input_encoder_outputs.encoder_last_hidden_state.detach()
                        mel_input_encoder_outputs_hidden_state0=mel_input_encoder_outputs.encoder_hidden_states[0].detach()
                        mel_input_encoder_outputs_hidden_state3=mel_input_encoder_outputs.encoder_hidden_states[3].detach()
                    ...
                    if self.config.mmd_input_type=='mean':
                        for i in range(dimension):
                            mmd_loss=mmd_loss+self.jmmd_loss(
                                (mel_input_encoder_outputs_hidden_state0[:,:,i],mel_input_encoder_outputs_hidden_state3[:,:,i],source[:,:,i]),
                                (outputs.encoder_hidden_states[0][:,:,i],outputs.encoder_hidden_states[3][:,:,i],target[:,:,i])
                            )
                        mmd_loss /= source.shape[-2]
                    else:
                ...
                if self.config.total_loss['mmd_bm'] != 0:
                    mmd_bm_loss=0
                    steps = torch.linspace(0, 1, 80)  # Normalize steps to [0, 1]
                    weights = -torch.flip((-1 + 0.8 * torch.exp(-3.2 * steps)), dims=[0])
                    for i in range(mel_spec.size(1)):
                        mmd_bm_loss = mmd_bm_loss + self.jmmd_mel_loss(
                            (mel_spec[:,i,:useful_length],),
                            (pred_mel[:,i,:useful_length],)
                        ) #* weights[i]
                    loss_components['mmd_bm'] = self.config.total_loss['mmd_bm']*mmd_bm_loss

                if self.config.total_loss['mse_whisper'] != 0:
                    mse_loss=MSELoss()(mel_input_encoder_outputs_hidden_state0, outputs.encoder_hidden_states[0])
                    mse_loss += MSELoss()(mel_input_encoder_outputs_hidden_state3, outputs.encoder_hidden_states[3])
                    mse_loss += MSELoss()(source, target)
                    loss_components['mse_whisper'] = self.config.total_loss['mse_whisper']*mmd_bm_loss

                if self.config.total_loss['ctc'] != 0:
                    input_lengths = torch.full(size=(ctc_out.shape[1],), fill_value=ctc_out.shape[0], dtype=torch.long)
                    ctc_labels = torch.stack(ctc_labels, dim=0)
                    token_mask = ctc_labels > 0
                    target_lengths = token_mask.sum(-1)
                    ctc_labels = ctc_labels.masked_select(token_mask)

                    assert len(ctc_labels) == sum(target_lengths)
                    ctc_loss = nn.CTCLoss(blank=0, reduction='mean', zero_infinity=True)(ctc_out, ctc_labels, input_lengths, target_lengths)
                    loss_components['ctc'] = self.config.total_loss['ctc']*ctc_loss
    ...
    def generate(
        ...
        inputs, _ = self.predict_mel(input_features=inputs,
