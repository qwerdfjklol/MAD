...
class DataCollatorBrainSpeechSeq2SeqWithPadding:
    ...
    def __call__(self, features: List[Dict[str, Union[List[int], torch.Tensor]]]) -> Dict[str, torch.Tensor]:
        ...
        frame_labels = torch.stack([torch.tensor(feature['frame_labels'], dtype=torch.int) for feature in features]).squeeze(1)
        ...
        batch['frame_labels'] = frame_labels
        ...
        # replace padding with -100 to ignore loss correctly
        labels = labels_batch["input_ids"].masked_fill(labels_batch.attention_mask.ne(1), -100)
        batch["ctc_labels"] = [feature["ctc_labels"] for feature in features]
