...
class Calculate_MCD(object):
    ...
    def average_mcd(self, ref_audio_file, syn_audio_file, cost_function, MCD_mode):
        ...
        if type(ref_audio_file) == str:
            ...
            loaded_ref_wav = ref_audio_file
            loaded_syn_wav = syn_audio_file
            mean_mcd = 0
 
            for ref_wav, syn_wav in zip(loaded_ref_wav, loaded_syn_wav):

                # if MCD_mode == "plain":
                #     # pad 0
                #     if len(ref_wav)<len(syn_wav):
                #         ref_wav = np.pad(ref_wav, (0, len(syn_wav)-len(ref_wav)))
                #     else:
                #         syn_wav = np.pad(syn_wav, (0, len(ref_wav)-len(syn_wav)))

                # extract MCEP features (vectors): 2D matrix (num x mcep_size)
                ref_mcep_vec = self.wav2mcep_numpy(ref_wav)
                syn_mcep_vec = self.wav2mcep_numpy(syn_wav)
...
if __name__ == '__main__':
    ...
    mcd_value = mcd_toolbox.calculate_mcd("/home/srt2023/bm/outputs/xps/97d170e1/pred0.wav", "/home/srt2023/bm/outputs/xps/97d170e1/pred0.wav")
    print(mcd_value)