...
from utils.process_utils import write_jsonlines
...
if args.noise==True:
    ...
    results_path = '/mnt/petrelfs/zhangchi1/m2t/results/'+state_dict.config.run_name
    mel_path = '/mnt/petrelfs/zhangchi1/m2t/predmel_output/'+state_dict.config.run_name
...
# metric_files = [ 'bleu', 'nltkbleu_sentence',  'mer', 'my_rouge','wer','word_info_lost','word_info_preserved', # 'sacrebleu' 'bert_score'
#                 'meteor','cer1'
#                 ]
metric_files = ['nltkbleu_sentence', 'my_rouge','wer', 'cer1']
# Load metrics
            ...
            with torch.no_grad():
                if not args.random_choice:
                    ...
                    if not args.teacher_forcing:
                        ...
                        model_output=model(input_features, subject_index=batch['subject_index'].cpu().cuda(), useful_length=1000, mel_spec=batch['mel_spec'].cuda(), labels=batch["labels"].cuda())
...
if args.random_choice:
    ...
    for metric in metrics:
        ...
        for key in result.keys():
            ...
            results[key] = result[key]
    print(f"Evaluation results: {results}")
    json_file_path = os.path.join(results_path, f'{result_basename}_{useful_length}.json')
    with open(json_file_path, 'w') as f:
        json.dump(results, f)
